...
const UpdatePackageForm = ({
  ...
  const handleSubmit = async (e) => {
    try {
      const response = await axios.put(
        `${HOST}/api/v1/admin/packages/${selectedPackage._id}`,
        formData
      );
      setPackages((prevPackages) =>
        prevPackages.map((pkg) =>
          pkg._id === selectedPackage._id ? response.data.updatedPackage : pkg
        )
      );
  ...
  return (
    <div className="flex justify-center mt-4">
      <form onSubmit={handleSubmit} className="w-[50vw]">
        <h2 className="text-2xl mb-4">Update the Package </h2>
        <div>
